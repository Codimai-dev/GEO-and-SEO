...
const AnalysisDetails: React.FC<AnalysisDetailsProps> = ({ report }) => {
  ...
  const [fixModal, setFixModal] = useState<{ open: boolean; type: string; title: string; suggestion: string; loading: boolean }>({
  ...
  const handleFixRequest = async (type: 'title' | 'description' | 'h1' | 'alt', contextData: any) => {
    ...
    try {
        ...
        const suggestion = await generateSpecificFix(type, {
            ...contextData,
            keywords,
            pageContext
        });
  ...
  const StatusIcon = ({ condition }: { condition: boolean }) => {
    return condition ? <CheckCircle className="text-green-500 w-4 h-4 md:w-5 md:h-5" /> : <XCircle className="text-red-500 w-4 h-4 md:w-5 md:h-5" />;
  };
      ...
      />
        ...
        <div className="bg-[#121212] rounded-xl border border-[#222] overflow-hidden">
          <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[#222] flex items-center gap-3 bg-[#0a0a0a]">
            <Search className="text-brand-500" size={18} />
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] md:text-xs px-2 py-1 rounded ${report.meta.title && report.meta.title.length <= 60 && report.meta.title.length >= 10 ? 'bg-green-900/20 text-green-400' : 'bg-red-900/20 text-red-400'}`}>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] md:text-xs px-2 py-1 rounded ${report.meta.description && report.meta.description.length <= 160 && report.meta.description.length >= 50 ? 'bg-green-900/20 text-green-400' : 'bg-red-900/20 text-red-400'}`}>
        ...
        <div className="bg-[#121212] rounded-xl border border-[#222] overflow-hidden">
             <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[#222] flex items-center gap-3 bg-[#0a0a0a]">
                <Hash className="text-brand-500" size={18} />
        ...
        <div className="bg-[#121212] rounded-xl border border-[#222] overflow-hidden">
            <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[#222] flex items-center gap-3 bg-[#0a0a0a]">
                <Share2 className="text-brand-500" size={18} />
                ...
                <div className="space-y-2">
                    ...
                    <div className="border border-gray-700 rounded overflow-hidden bg-[#000] max-w-sm mx-auto md:mx-0 w-full shadow-lg">
                        ...
                        <div className="p-3 bg-[#18191a]">
                            ...
                            <p className="text-[#e4e6eb] font-bold text-xs md:text-sm truncate leading-tight mb-1">{report.social.ogTitle || report.meta.title || 'Missing Title'}</p>
                            <p className="text-[#b0b3b8] text-[10px] md:text-xs line-clamp-2">{report.social.ogDescription || report.meta.description || 'Missing Description'}</p>
                        </div>
                    </div>
                </div>
                 {/* Twitter / X */}
                    ...
                    <div className="border border-gray-700 rounded-xl overflow-hidden bg-black max-w-sm mx-auto md:mx-0 w-full shadow-lg">
                        <div className="h-32 md:h-40 bg-[#222] relative flex items-center justify-center overflow-hidden group">
                             {report.social.twitterImage || report.social.ogImage ? (
                                <img src={report.social.twitterImage || report.social.ogImage || ''} alt="Twitter Preview" className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                            ) : (
                                <span className="text-gray-600 text-xs">No Image</span>
                            )}
                        </div>
                        <div className="p-3">
                            <p className="text-white font-bold text-xs md:text-sm truncate mb-1">{report.social.twitterTitle || report.social.ogTitle || report.meta.title || 'Missing Title'}</p>
                            <p className="text-gray-500 text-[10px] md:text-xs line-clamp-2">{report.social.twitterDescription || report.social.ogDescription || report.meta.description || 'Missing Description'}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div className="bg-[#121212] rounded-xl border border-[#222] overflow-hidden">
                <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[#222] flex items-center gap-3 bg-[#0a0a0a]">
                    <Image className="text-brand-500" size={18} />
                ...
                <div className="p-4 md:p-6">
                    ...
                    <div className="flex items-center justify-between py-2 border-b border-[#222]">
                        ...
                        <div className="flex items-center gap-2">
                            <span className={`${report.images.withoutAlt > 0 ? 'text-red-400' : 'text-white'} font-bold text-sm`}>{report.images.withoutAlt}</span>
