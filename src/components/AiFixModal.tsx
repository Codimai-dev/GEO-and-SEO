...
const AiFixModal: React.FC<AiFixModalProps> = ({ isOpen, onClose, onRetry, title, suggestion, isLoading, type }) => {
  ...
  const isError = suggestion.toLowerCase().includes('failed') || suggestion.toLowerCase().includes('error');
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-[#121212] w-full max-w-2xl rounded-xl border border-red-500/30 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          {isLoading ? (
            ...
            <div className="w-full h-full flex flex-col">
              <p className="text-xs text-gray-500 uppercase font-bold mb-2 tracking-wider">
                {isError ? 'Error' : 'Suggested Fix'}
              </p>
              <div className={`bg-[#000] border rounded-lg p-4 relative group flex-1 overflow-hidden flex flex-col ${isError ? 'border-red-500/50' : 'border-[#333]'}`}>
        ...
        {/* Footer */}
        {!isLoading && (
          <div className="p-4 border-t border-[#222] bg-[#0a0a0a] flex justify-between items-center flex-shrink-0">
            {onRetry && (
              <button
                onClick={onRetry}
