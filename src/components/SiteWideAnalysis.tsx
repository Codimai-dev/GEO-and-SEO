...
const getIssuesForPage = (page: CrawledPage): SimulatedIssue[] => {
    // Use real issues if available
    if (page.issueDetails && page.issueDetails.length > 0) {
        return page.issueDetails.map(issue => ({
    ...
    // Fallback to simulation only if no details are present (legacy support)
...
const SiteWideAnalysis: React.FC<SiteWideAnalysisProps> = ({ pages }) => {
    ...
    const [fixModal, setFixModal] = useState<{
        ...
        loading: boolean;
        type: string;
        currentPage: CrawledPage | null;
    }>({
        open: false, title: '', suggestion: '', loading: false, type: 'general', currentPage: null
    });
    ...
    const handleAiFix = async (page: CrawledPage, issueType: string = 'general') => {
        ...
        else if (issueType === 'content_ideas' || issueType === 'content') title = 'Content Expansion Ideas';
        else if (issueType === 'images') title = 'Fixing Image Alt Text';
        else if (issueType === 'canonical') title = 'Adding Canonical Tag';
        else title = `Fixing ${page.url.split('/').pop() || 'Page'}`;

        setFixModal({ open: true, title, suggestion: '', loading: true, type: issueType, currentPage: page });

        try {
            const result = await apiService.getSpecificFix(issueType, {
                url: page.url,
    ...
    const handleRetry = () => {
        if (fixModal.currentPage) {
            handleAiFix(fixModal.currentPage, fixModal.type);
        }
    };
                                        ...
                                        <div key={idx} className="bg-[#0a0a0a] border border-[#333] p-4 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-gray-600 transition-colors group">
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${issue.severity === 'High'
